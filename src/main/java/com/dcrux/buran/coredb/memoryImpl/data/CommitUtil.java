...
public class CommitUtil {
    public Set<PreparedComitInfo> generateOidsFromIoids(long senderId, final Set<IncNid> incNids,
        ...
        /* Update old version */
                ...
                final PreparedComitInfo pci =
                        new PreparedComitInfo(new NidVer(oid, NodeSerie.FIRST_VERSION), incNid,
                                classId, false, null, incNode.getReceiverId(), incNode,
                                newNodeSerie);
    ...
    public void validateEdgeTarget(final Set<PreparedComitInfo> pciEntry,
            final IncubationEdge incEdge, long receiverId, DataReadApi drApi) {
        final IIncEdgeTarget target = incEdge.getTarget();
        switch (target.getIncType()) {
            case externalUnversioned:
                System.out.println(
                        "TODO: Hier muss überprüft werden, ob die userId bekannt ist! (Der Rest " +
            ...
            case externalVersioned:
                ...
                break;
            case unversioned:
                if (!drApi.oidExistsInCurrentOrHistory(receiverId,
                        ((UnversionedEdTarget) target).getNid())) {
                    throw new IllegalStateException("Edge target not found");
                }
                break;
            case versioned:
                if (!drApi.oidExistsInCurrentOrHistory(receiverId,
                        ((VersionedEdTarget) target).getNid())) {
                    throw new IllegalStateException("Edge target not found");
                }
                break;
            case unversionedInc:
          /* Has to be in incubation */
                NidVer found =
                        findOidInIncOids(pciEntry, ((IncUnversionedEdTarget) target).getInid());
                if (found == null) {
                    throw new IllegalStateException(
                            "A given edge-inc-oid is not found in incubation");
                }
                break;
            case versionedInc:
                   /* Has to be in incubation */
                NidVer foundVer =
                        findOidInIncOids(pciEntry, ((IncVersionedEdTarget) target).getInid());
                if (foundVer == null) {
                    throw new IllegalStateException(
                            "A given edge-inc-oid is not found in incubation");
                }
    ...
    /* Check private edge */
        for (Map.Entry<Short, EdgeClass> edgeEntry : nc.getEdgeClasses().entrySet()) {
            ...
            for (Map.Entry<IncNode.EdgeIndexLabel, IncubationEdge> edgeInstance : pci.getIncNode()
        ...
        for (final PreparedComitInfo pciEntry : prepComInfo) {
            if (pciEntry.isUpdate()) {
                final boolean exists =
                        drApi.existsInCurrent(pciEntry.getReceiverId(), pciEntry.getOidToUpdate());
                if (!exists) {
    ...
    /* Check node targets */
        for (final PreparedComitInfo pciEntry : prepComInfo) {
            for (final Map.Entry<IncNode.EdgeIndexLabel, IncubationEdge> eEntry : pciEntry
                    .getIncNode().getIncubationEdges().entrySet()) {
        ...
        switch (target.getIncType()) {
            ...
            case externalVersioned:
                finalTarget = new ExtEdgeImplTarget(((ExtVersionedEdTarget) target).getUserId(),
                        ((ExtVersionedEdTarget) target).getNid(),
                        ((ExtVersionedEdTarget) target).getVersion());
                break;
            case unversioned:
                final UnversionedEdTarget unversionedEdTarget = (UnversionedEdTarget) target;
                finalTarget = new UnversionedEdgeImplTarget(
                        accountNodes.getNodeSerieByOid(unversionedEdTarget.getNid(), false));
        /* Add to in-node */
        ...
        /* Add to in node */
            ...
            case unversionedInc:
                final IncUnversionedEdTarget incUnversionedEdTarget =
                        (IncUnversionedEdTarget) target;
                final PreparedComitInfo pci =
                        findPrepComInfoByIncOid(prepComInfo, incUnversionedEdTarget.getInid());
                assert (pci != null);
            ...
            case versionedInc:
                ...
                final PreparedComitInfo pciTwo =
                        findPrepComInfoByIncOid(prepComInfo, incVersionedEdTarget.getInid());
                assert (pciTwo != null);
